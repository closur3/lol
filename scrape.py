...
"""
    ...
    for t in TOURNAMENTS:
        ...
        for _, team, s in rows:
            bo3_r = rate(s["bo3_full"], s["bo3_total"])
            bo5_r = rate(s["bo5_full"], s["bo5_total"])
            mw = rate(s["match_win"], s["match_total"])
            gw = rate(s["game_win"], s["game_total"])
            ...
            html += (
                ...
                f"<td style='background:{color_by_rate(bo3_r)}'>{pct(bo3_r)}</td>"
                f"<td>{s['bo5_full']}/{s['bo5_total']}</td>"
                f"<td style='background:{color_by_rate(bo5_r)}'>{pct(bo5_r)}</td>"
                f"<td>{pct(mw)}</td>"
                f"<td>{pct(gw)}</td>"
                f"<td style='background:{streak_color}'>{streak}</td>"
                "</tr>"
            )
